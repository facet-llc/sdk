from __future__ import annotations

from collections.abc import Mapping
from typing import Any, TypeVar, BinaryIO, TextIO, TYPE_CHECKING, Generator

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset

from typing import cast

if TYPE_CHECKING:
  from ..models.bid_summary import BidSummary





T = TypeVar("T", bound="GetBidStatusResponse")



@_attrs_define
class GetBidStatusResponse:
    """ 
        Attributes:
            bid_count (int):
            bids (list[BidSummary]):
            is_high_bidder (bool):
     """

    bid_count: int
    bids: list[BidSummary]
    is_high_bidder: bool
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)





    def to_dict(self) -> dict[str, Any]:
        from ..models.bid_summary import BidSummary
        bid_count = self.bid_count

        bids = []
        for bids_item_data in self.bids:
            bids_item = bids_item_data.to_dict()
            bids.append(bids_item)



        is_high_bidder = self.is_high_bidder


        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update({
            "bid_count": bid_count,
            "bids": bids,
            "is_high_bidder": is_high_bidder,
        })

        return field_dict



    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        from ..models.bid_summary import BidSummary
        d = dict(src_dict)
        bid_count = d.pop("bid_count")

        bids = []
        _bids = d.pop("bids")
        for bids_item_data in (_bids):
            bids_item = BidSummary.from_dict(bids_item_data)



            bids.append(bids_item)


        is_high_bidder = d.pop("is_high_bidder")

        get_bid_status_response = cls(
            bid_count=bid_count,
            bids=bids,
            is_high_bidder=is_high_bidder,
        )


        get_bid_status_response.additional_properties = d
        return get_bid_status_response

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
