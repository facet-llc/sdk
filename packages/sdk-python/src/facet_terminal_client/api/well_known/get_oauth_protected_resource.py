...
def sync_detailed(
    *,
    ...
    """ OAuth 2.0 Protected Resource Metadata (RFC 9728) for this host's MCP endpoint. Names the canonical
    resource URI (<host>/ucp/mcp), the trusted authorization servers that may mint tokens for it, and
    the supported scopes, so a stock MCP client can start an OAuth flow from the 401 challenge. KYA
    remains a fully sufficient credential; this path is additive. Returns 404 until the operator enables
    UCP via FACET_UCP_ENABLED.
