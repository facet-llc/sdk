...
from ...models.open_api_document import OpenApiDocument
...
def _parse_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> FacetErrorEnvelope | OpenApiDocument | None:
    if response.status_code == 200:
        response_200 = OpenApiDocument.from_dict(response.json())
...
def _build_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> Response[FacetErrorEnvelope | OpenApiDocument]:
    return Response(
...
) -> Response[FacetErrorEnvelope | OpenApiDocument]:
    """ Canonical OpenAPI 3.1 spec (JSON). Public, cached for 1 hour at the edge. SDK generators MUST
    consume this URL so per-merchant overlay output is honoured.
...
def sync(
    *,
...
) -> FacetErrorEnvelope | OpenApiDocument | None:
    """ Canonical OpenAPI 3.1 spec (JSON). Public, cached for 1 hour at the edge. SDK generators MUST
    consume this URL so per-merchant overlay output is honoured.
    ...
    Returns:
        FacetErrorEnvelope | OpenApiDocument
     """
