...
def sync_detailed(
    *,
    ...
    """ UCP checkout GET. Rebuilds the full checkout session (payment_handlers + totals + line_items) from
    the sealed reservation plus the site's live rail config, so a read is the SAME shape CREATE
    returned, with the status mapped from the reservation lifecycle (ready_for_complete for an active
    hold, completed once settled, canceled once canceled/expired). Owner-scoped: a checkout owned by a
    different agent, and one that does not exist, both return the SAME 404, so an id can never be probed
    for existence. Public + activation-exempt; 404 until FACET_UCP_ENABLED.
