...
def sync_detailed(
    *,
    ...
    """ UCP cart GET. Retrieves the current cart snapshot (line_items + re-priced totals). Owner-scoped: a
    cart owned by a different agent, and a cart that does not exist, both return the SAME 404, so a cart
    id can never be probed for existence. Public + activation-exempt; 404 until FACET_UCP_ENABLED.
