...
from ...models.ucp_cart_update_request import UcpCartUpdateRequest
...
def sync_detailed(
    *,
    ...
    """ UCP cart UPDATE. Full replacement of the cart's line_items, RE-PRICED server-side; the cart id is
    unchanged. Owner-scoped (a foreign or non-active cart returns 404). Public + activation-exempt; 404
    until FACET_UCP_ENABLED.

    Args:
        accept (str | Unset):  Default: 'application/vnd.facet+json; version=0.2'.
        x_facet_trace_id (str | Unset):
        body (UcpCartUpdateRequest): A UCP cart update request (POST /ucp/v1/carts/:id). Full
            replacement of the cart's line items, re-priced server-side. Keeps the same cart id.
