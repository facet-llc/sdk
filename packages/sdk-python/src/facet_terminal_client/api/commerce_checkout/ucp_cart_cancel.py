...
from ...models.ucp_cart_cancel_request import UcpCartCancelRequest
...
def sync_detailed(
    *,
    ...
    """ UCP cart CANCEL. Cancels the cart (idempotent: a second cancel is a no-op). Owner-scoped (a foreign
    or missing cart returns 404). The cart schema has no status field, so cancellation is surfaced via a
    messages[] note beside the snapshot. Public + activation-exempt; 404 until FACET_UCP_ENABLED.

    Args:
        accept (str | Unset):  Default: 'application/vnd.facet+json; version=0.2'.
        x_facet_trace_id (str | Unset):
        body (UcpCartCancelRequest): A UCP cart cancel request (POST /ucp/v1/carts/:id/cancel).
            The body is empty; the cart id comes from the path.
