...
def _parse_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> FacetErrorEnvelope | ReadyResponse | None:
    ...
    if response.status_code == 503:
        response_503 = ReadyResponse.from_dict(response.json())
...
def _build_response(*, client: AuthenticatedClient | Client, response: httpx.Response) -> Response[FacetErrorEnvelope | ReadyResponse]:
    return Response(
...
) -> Response[FacetErrorEnvelope | ReadyResponse]:
    """ Readiness probe. Checks that critical dependencies (Supabase) are reachable; returns 200 { status:
    'ready' } when all are healthy, 503 { status: 'not_ready' } when any is down. Distinct from
    /v1/health (liveness). Intended for monitoring readiness checks — not for kill health-checks.
...
def sync(
    *,
...
) -> FacetErrorEnvelope | ReadyResponse | None:
    """ Readiness probe. Checks that critical dependencies (Supabase) are reachable; returns 200 { status:
    'ready' } when all are healthy, 503 { status: 'not_ready' } when any is down. Distinct from
    /v1/health (liveness). Intended for monitoring readiness checks — not for kill health-checks.
    ...
    Returns:
        FacetErrorEnvelope | ReadyResponse
     """
